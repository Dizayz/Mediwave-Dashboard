...
imageRef.value = document.querySelector(".img")
const toggleVisibility = () => {
    ...
    if (ShowToolTip.value) {
        setTimeout(() => {
            // Use optional chaining to safely access and modify the style
            imageRef.value?.style.setProperty('margin-right', '10px');
            
        }, 200);
    } else {
        setTimeout(() => {
            // Use optional chaining here as well
            imageRef.value?.style.setProperty('margin', 'unset');
...
const toggleVisibilityWithButton = () => {
...
<template>
    <aside>
        ...
        <div class="close-nav">
            <p>Réduire La Barre</p>
            <v-tooltip v-if="!ShowToolTip" activator="parent" location="start">Agrandir la barre</v-tooltip>
            <button @click="toggleVisibilityWithButton()"><svg fill="#ababab" version="1.1" id="Capa_1"
...
.show {
    ...
    opacity: 1;
    overflow: hidden;
    transition: max-height 0.8s, max-width 1s, opacity 0.8s ease-in-out;
    white-space: nowrap;
}
...
p {
    ...
    opacity: 0;
    overflow: hidden;
    transition: max-height 0.5s, max-width 1s, opacity 0.5s ease-in-out;
}
